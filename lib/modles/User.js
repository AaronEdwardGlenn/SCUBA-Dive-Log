...
schema.statics.authorize = async function({ email, password }) {
  ...
  if(!user) {
    const err = new Error('This email is invalid. Or maybe password. You will never know.'); 
    err.status = 401; 
    throw err; 
  }
  ...
  if(!validPassword){
    const err = new Error('That email or password is not correct. Sort it out.'); 
    err.status = 401; 
    throw err; 
  }
  return user; 
...
schema.methods.authToken = function() {
  return jwt.sign(this.toJSON(), process.env.APP_SECRET || ('sweet'), {
    expiresIn: '6h'
  }); 
};
